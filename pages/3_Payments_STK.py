...
with col1:
    ...
    if not counts.empty:
        st.plotly_chart(charts.pie_chart(counts, "status", "count"), use_container_width=True)
    else:
...
with col2:
    ...
    if not counts.empty:
        st.plotly_chart(charts.pie_chart(counts, "mode", "count"), use_container_width=True)
    else:
        st.info("No payment data available.")
...
if not daily.empty:
    st.plotly_chart(charts.bar_chart(daily, "day", "amount"), use_container_width=True)
else:
